...
class StudentMigrate(models.TransientModel):
    ...
    course_from_id = fields.Many2one('op.course', 'From Semester', required=True)
    course_to_id = fields.Many2one('op.course', 'To Semester', required=True)
    ...
    def _check_admission_register(self):
        for record in self:
            if record.course_from_id == record.course_to_id:
                raise ValidationError(_("From Semester must not be same \
                as To Semester!"))
            ...
            if record.course_from_id.parent_id:
                if record.course_from_id.parent_id != \
                    ...
                    raise ValidationError(_(
                        "Can't migrate, As selected semesters don't \
                        share same parent semester!"))
            else:
                raise ValidationError(_(
